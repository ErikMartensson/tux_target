#!/bin/bash
#
# Build Tux Target Client Only
#
# This script builds only the game client to a separate build-client directory.
# Usage: ./scripts/build-client.sh [--clean] [--skip-post-build]
#
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}  Tux Target Client Build${NC}"
echo -e "${BLUE}=========================================${NC}"
echo ""
...
echo -e "${GREEN}Configuring CMake (Client only)...${NC}"
cmake .. \
    -DBUILD_CLIENT=ON \
    -DBUILD_SERVER=OFF \
    -DWITH_STATIC=ON \
    ...
    -DCURL_DIR="${TUXDEPS_PREFIX_PATH}/curl" \
    -DLUA_DIR="${TUXDEPS_PREFIX_PATH}/lua"
...
if [ -f "${BUILD_DIR}/bin/Release/tux-target.exe" ]; then
    SIZE=$(stat -c%s "${BUILD_DIR}/bin/Release/tux-target.exe" 2>/dev/null || stat -f%z "${BUILD_DIR}/bin/Release/tux-target.exe" 2>/dev/null)
    echo -e "${GREEN}Client executable: ${BUILD_DIR}/bin/Release/tux-target.exe (${SIZE} bytes)${NC}"
else
    echo -e "${RED}Warning: Client executable not found${NC}"
fi
...
if [ "$SKIP_POST_BUILD" = false ]; then
    ...
    "${SCRIPT_DIR}/post-build.sh" --client-only --build-dir "${BUILD_DIR}/bin/Release"
fi

echo ""
echo -e "${GREEN}=========================================${NC}"
echo -e "${GREEN}  Client build finished!${NC}"
echo -e "${GREEN}=========================================${NC}"
echo ""
echo "To run the client:"
echo "  cd ${BUILD_DIR}/bin/Release && ./tux-target.exe"
echo ""
