...
function CLevel:init()
	...
	self:addModuleNP("snow_island", CVector(-9, -8.5, 3.5))
	local m = self:addModuleNP("snow_island2", CVector(15,-30,2)) m:setRotation(CAngleAxis(0,0,1,1.58))
	local m = self:addModuleNP("snow_island3", CVector(-2,-40,2)) m:setRotation(CAngleAxis(0,0,1,2.58))
	local m = self:addModuleNP("snow_island2", CVector(-10,-20,2)) m:setRotation(CAngleAxis(0,0,1,-1.58))
	local m = self:addModuleNP("snow_island3", CVector(10,-2,2)) m:setRotation(CAngleAxis(0,0,1,-1.58))
	...
	local m = self:addModuleNPSS("snow_box", CVector(2.5,-5.85,6.8), 0, CVector(200, 1, 50))
	m:setTexture(0, "donuts_ad")
	m:setColor(CRGBA(255,255,255))

	-- right wall to avoid cheat (thanks bady)
	self:addModuleNPSS("snow_box", CVector(-0.95, -8.2, 4.5), 0, CVector(1, 225, 50))
	...
	-- vertical wall
	local m = self:addModuleNPSS("snow_box", CVector(-1.2,-8.5, 2.57), 0, CVector(0.2, 20, 20))
